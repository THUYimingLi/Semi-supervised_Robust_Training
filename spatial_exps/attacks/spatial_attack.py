    ...
    for tx, ty, r in grid:
        ...
        if self.use_kl:
          curr_dict = {self.model.x_input: x,
                       self.model.y_input: np.argmax(y, axis=-1),
                       self.model.y_pred_input: y,
                       self.model.is_training: False,
                       self.model.weights: [1. for i in range(len(x))],
                       self.model.kl_weights: [1. for i in range(len(x))],
        ...
        cur_xent, cur_correct, pred = sess.run([ self.model.y_kl if self.use_kl else self.model.y_xent,
                                          self.model.correct_prediction, self.model.softmax], 
